...
// Convention: Use an extension to group a type's functionality Think of it this way. When we define the type, we only provide what information is needed to define the type's identity. In our case, over here an enum with members is all we need to define the Story type. The computed properties that we're about to add don't really contribute to this identity, but instead they rely on the identity to provide additional data. So instead of adding the computed properties to the type declaration we're going to add them to an extension.
